...
import { Button, Card, CardContent, Divider, Menu, MenuItem, Step, StepLabel, Stepper } from '@mui/material';
import { useEffect, useState } from 'react';
import { addOrder, addPayment, cancelOrder, getOrder, getOrderDetail } from '../../../services/api';
...
export default function Order() {
  ...
  const handleClick = () => {
    ...
    if (activeStep === 1) {
      if (userPayment < orderDetail.price) {
        ...
        addPayment(orderDetail.orderNumber , {'amount': parseInt(userPayment)}).then((res) => {
          setIsLoading(false)
          if (res.error !== null)
            throw new error();
        }).catch((err) => {
  ...
  const handleCancel = () => {
    setIsLoading(true)
    cancelOrder(orderDetail.orderNumber).then((res) => {
      if (error == null)
        alert('Canceled!')
    }).catch((err) => {
      alert('Error!')
                ...
                <div key={item.orderNumber} onClick={() => handleOrderClick(item.orderNumber)}>
                  <MenuItem>
                    <div>
                      ...
                      <div>
                        status: <span
                        style={{color: item.status === 'PENDING_PAYMENT' ? 'orange': item.status === 'CANCELLED' ? 'red' : 'green'}}
                      > {item.status}
                              </span>
                      </div>
                    </div>
                    ...
                    <div>
                      {
                        ...
                        orderDetail.details.map((dishItem, index) => {
                          return (
                            <OrderCard key={index.toString()} id={dishItem.dishId} quantity={dishItem.quantity}/>
                          )
                        })
                      }
                    </div>
            ...
            >
              {
                isLoading ?
                  ...
                  <div style={{ textAlign: 'left'}}>
                    <Card>
                      <CardContent>
                        Combo: {orderDetail?.combo ?? 'None'}
                      </CardContent>
                    </Card>
                    <Card style={{marginTop: '5px'}}>
                      <CardContent>
                        Total Payment: ₱{orderDetail?.price}
                      </CardContent>
                    </Card>
